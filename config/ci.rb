...
CI.run do
  ...
  if arch_linux?
    step "Tests: checking for chromium on arch", %([[ -f "/usr/bin/chromium"]] && echo "local chromium found")
    step "Tests: checking for firefox on arch", %([[ -f "/usr/bin/firefox"]] && echo "local firefox found")
  else
