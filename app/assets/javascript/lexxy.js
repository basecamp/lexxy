...
class ImportStyledTextNode extends Xn {
  static importDOM() {
    return {
      mark: () => ({
...
function extendTextNodeConversion(conversionName, styler= (textNode => textNode)) {
  return (element) => {
    const textConverter = Xn.importDOM()?.[conversionName]?.(element);
