...
class CommandDispatcher {
  ...
  dispatchApplyHeadingFormat(tag) {
    ...
    if (xs(selection.anchor.getNode())) {
      if (tag) selection.insertNodes([ Mt$2(tag) ]);
      return
    }

    if (tag) {
      this.contents.insertNodeWrappingEachSelectedLine(() => Mt$2(tag));
    } else {
  ...
  dispatchRotateHeadingFormat() {
    ...
    if (xs(selection.anchor.getNode())) {
      selection.insertNodes([ Mt$2(headings[0]) ]);
      return
    }

    const currentHeading = this.selection.nearestNodeOfType(Tt$2);
...
class Selection {
  ...
  getFormat() {
    ...
    return {
      ...
      headingTag: vt$4(anchorNode, Tt$2)?.getTag(),
...
class HeadingDropdown extends ToolbarDropdown {
  ...
  #handleToggle({ newState }) {
    if (newState === "open") {
      this.editor.getEditorState().read(() => {
        this.#updateHeadingButtonStates($r());
      });
    }
  }
