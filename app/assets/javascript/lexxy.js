...
class ImageGalleryNode extends Ci {
        ...
        return {
          conversion: () => {
            return {
              ...
              after: children => kt$6(children, $isActionTextAttachmentNode)
            }
          },
          priority: 2
        }
  ...
  select(anchorOffset, _focusOffset) {
    if (anchorOffset === undefined) {
      ...
      const selectionTarget = targetChild.isSelected(Lr()) ? targetChild.getPreviousSibling() : targetChild;
  ...
  splitAroundInvalidChild() {
    for (const child of $t$3(this)) {
      if (!this.isValidChild(child)) {
        const poppedNode = child.isParentRequired() ? bt$6(child, Li) : child;
        const [ topGallery ] = ms(this, poppedNode.getIndexWithinParent());
...
function $findOrCreateGalleryFor(node) {
...
class GalleryUploader extends Uploader {
  ...
  get #imageNodes() {
    return this.nodes.filter(node => node.isPreviewableImage)
  }
...
class Contents {
  ...
  #findAdjacentNodeTo(nodes) {
    ...
    const previousOrNextSibling = previousSibling || lastNode && mt$2(lastNode)?.[0];
    ...
    return Kl({
      ...
      register(editor) {
        return U$4(
          ...
          editor.registerNodeTransform(Ni, $removeUneededProvisionalParagraphs),
...
class AttachmentsExtension extends LexxyExtension {
  ...
  get lexicalExtension() {
    return Kl({
      ...
      register(editor) {
        return U$4(
          editor.registerCommand(ie$1, $ensureGalleryChildSelection, Bi),
          editor.registerCommand(Ne$2, $insertParagraphAfterNode, Bi),
          editor.registerCommand(ke$2, () => $selectSibling("up"), Bi),
          editor.registerCommand(Te$2, () => $selectSibling("down"), Bi),
          editor.registerCommand(Se$1, () => $selectSiblingAtEdge("start"), Bi),
          editor.registerCommand(xe$1, () => $selectSiblingAtEdge("end"), Bi)
        )
      }
    })
  }
...
function $ensureGalleryChildSelection() {
  const selection = Lr();
  if (!yr(selection) || selection.anchor.type !== "element") return false
...
function $insertParagraphAfterNode() {
  const selection = Lr();
  if (!xr(selection)) return false

  const focusNode = selection.getNodes()?.at(0);
  const isAttachment = wt$5(focusNode, ActionTextAttachmentNode);
...
function $selectSiblingAtEdge(edge = "start") {
  const selection = Lr();
  const isNodeSelection = xr(selection);
  if (!yr(selection) && !isNodeSelection) {
    return false
  }
  ...
  const isAtEdge = isNodeSelection || isEdgeNode && (edge === "start" ? selection.anchor.offset === 0 : _$2(selection.anchor));
