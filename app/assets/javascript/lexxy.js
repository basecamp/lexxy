  ...
  get lexicalExtension() {
    const extension = Yl({
      ...
      register(editor, config) {
        ...
        return ec(
          editor.registerCommand(TOGGLE_HIGHLIGHT_COMMAND, (styles) => $toggleSelectionStyles(styles, editor), Gi),
          editor.registerCommand(REMOVE_HIGHLIGHT_COMMAND, () => $toggleSelectionStyles(BLANK_STYLES, editor), Gi),
          ...
          editor.registerMutationListener(lr, () => {
...
function $isSelectionInCodeBlock(selection) {
  const anchorNode = selection.anchor.getNode();
  const codeNode = Q$1(anchorNode) ? anchorNode : anchorNode.getParent();
  return Q$1(codeNode)
}

function $getCodeNodeFromSelection(selection) {
  const anchorNode = selection.anchor.getNode();
  return Q$1(anchorNode) ? anchorNode : anchorNode.getParent()
}
...
function $patchSelectionStyle(selection, patch) {
  const prevStyles = b$3(selection.style || "");
  ...
  selection.setStyle(R$3(newStyles));
}
