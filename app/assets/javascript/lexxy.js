...
function styleFilterHook(_currentNode, hookEvent) {
  if (hookEvent.attrName === "style" && hookEvent.attrValue) {
    const styles = { ...B$1(hookEvent.attrValue) };
    ...
    if (Object.keys(sanitizedStyles).length) {
      hookEvent.attrValue = F$1(sanitizedStyles);
    } else {
...
function extendTextNodeConversion(conversionName, callback = (textNode => textNode)) {
  return extendConversion(Jn, conversionName, (conversionOutput, element) => ({
    ...conversionOutput,
    forChild: (lexicalNode, parentNode) => {
      const originalForChild = conversionOutput?.forChild ?? (x => x);
      let childNode = originalForChild(lexicalNode, parentNode);

      if (Qn(childNode)) childNode = callback(childNode, element) ?? childNode;
      return childNode
    }
  }))
}
...
class AttachmentGalleryNode extends fi {
  static getType() {
...
function $createAttachmentGalleryNode() {
  return new AttachmentGalleryNode()
}
...
class CommandDispatcher {
  ...
  dispatchInsertQuoteBlock() {
    this.contents.toggleNodeWrappingAllSelectedNodes((node) => wt$1(node), () => xt$2());
  }
  ...
  dispatchInsertCodeBlock() {
    this.editor.update(() => {
      if (this.selection.hasSelectedWordsInSingleLine) {
        ...
        this.contents.toggleNodeWrappingAllSelectedLines((node) => I(node), () => new K("plain"));
      }
    });
  }
...
class Selection {
  ...
  #processSelectionChangeCommands() {
    ...
    this.editor.registerCommand(Ne, this.#selectPreviousTopLevelNode.bind(this), Ii);
    this.editor.registerCommand(we, this.#selectNextTopLevelNode.bind(this), Ii);
...
class FormatEscaper {
  ...
  #escapeFromList(anchorNode) {
    ...
    const isInBlockquote = blockquote && wt$1(blockquote);
  ...
  #escapeFromBlockquote(anchorNode) {
    ...
    if (!blockquote || !wt$1(blockquote)) return
  ...
  #splitBlockquoteWithList(blockquote, parentList, emptyListItem, listItemsAfter) {
    ...
    const middleParagraph = Pi();
    blockquote.insertAfter(middleParagraph);

    const newList = st$2(parentList.getListType());

    const newBlockquote = xt$2();
  ...
  #removeTrailingEmptyListItems(list) {
    ...
    for (let i = items.length - 1; i >= 0; i--) {
      const item = items[i];
      if (Z$2(item) && this.#isNodeEmpty(item)) {
        item.remove();
      } else {
        break
      }
...
class Contents {
  ...
  #withoutTrailingEmptyParagraphs(elements) {
    ...
    while (lastNonEmptyIndex >= 0) {
      ...
      if (!Fi(element) || !this.#isElementEmpty(element)) {
        break
      }
      lastNonEmptyIndex--;
    }
  ...
  #isElementEmpty(element) {
    ...
    return children.length === 0 || children.every(child => Fn(child))
  }
  ...
  #removeStandaloneEmptyParagraph() {
    ...
    if (root.getChildrenSize() === 1) {
      ...
      if (firstChild && Fi(firstChild) && this.#isElementEmpty(firstChild)) {
        firstChild.remove();
      }
    }
  }
...
class Highlighter {
  ...
  toggle(styles) {
    this.editor.update(() => {
      ...
      L$2(node => this.#syncHighlightWithStyle(node));
    });
  }
  ...
  #toggleSelectionStyles(styles) {
    ...
    for (const property in styles) {
      const oldValue = Y$3(selection, property);
  ...
  #hasHighlightStyles(node) {
    const styles = B$1(node.getStyle());
    return !!(styles.color || styles["background-color"])
  }
}

class HighlightNode extends Jn {
  $config() {
    return this.config("highlight", { extends: Jn })
  }
...
class TrixTextNode extends Jn {
  $config() {
    return this.config("trix-text", { extends: Jn })
  }
  ...
  static importDOM() {
    return {
      ...
      pre: (element) => onlyPreLanguageElements(element, {
        conversion: extendConversion(K, "pre", applyLanguage),
        priority: 1
      })
    }
  }
...
class LexicalEditorElement extends HTMLElement {
  ...
  #registerComponents() {
    Mt(this.editor);
    this.historyState = E();
    v(this.editor, this.historyState, 20);
    _t$3(this.editor);
  ...
  #handleFocus() {
    ...
    this.editor.registerCommand(Ge, () => { dispatch(this, "lexxy:blur"); }, zi);
    this.editor.registerCommand(qe, () => { dispatch(this, "lexxy:focus"); }, zi);
  }
...
class ColorDialog extends HTMLElement {
  ...
  #updateColorButtonStates(selection) {
    const textColor = Y$3(selection, "color", "");
    const backgroundColor = Y$3(selection, "background-color", "");
...
class LexicalPromptElement extends HTMLElement {
  ...
  #registerKeyListeners() {
    ...
    this.keyListeners.push(this.#editor.registerCommand(Ne, this.#handleArrowUp.bind(this), Ki));
    this.keyListeners.push(this.#editor.registerCommand(we, this.#handleArrowDown.bind(this), Ki));
  }
