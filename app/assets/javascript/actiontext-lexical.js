    ...
    if (unfurlData.title) {
      this.editor.update(() => {
        ...
        function traverseNodes(node) {
          if (Qn(node) && node.getTextContent() === linkUrl) { textNodes.push(node); }
          if (di(node)) {
...
class LexicalPromptElement extends HTMLElement {
  ...
  async #showPopover() {
    ...
    this.unregisterTabListener = this.#editor.registerCommand(Pe, this.#handleSelectedOption.bind(this), Ki);
  }
